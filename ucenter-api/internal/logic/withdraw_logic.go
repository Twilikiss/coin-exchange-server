// Package logic
// @Author twilikiss 2024/8/13 0:11:11
package logic
...
func (w *Withdraw) QueryWithdrawCoin(t *types.WithdrawReq) ([]*types.WithdrawWalletInfo, error) {
	// 先从我们的context上下文中拿到我们的userId
	userId := w.ctx.Value("userId").(int64)

	// 1. 查询所有的coin信息
	coinList, err := w.svcCtx.MarketRPC.FindAllCoin(w.ctx, &market.MarketReq{})
	if err != nil {
		logx.Error("查询coinList失败， err=", err)
		return nil, err
	}
	...
	for i, wallet := range walletList.List {
		...
		err = copier.Copy(&addressList, addressSimpleList.List)
		if err != nil {
...
func (w *Withdraw) SendCode(req *types.WithdrawReq) (interface{}, error) {
...
func (w *Withdraw) Record(req *types.WithdrawReq) (*pages.PageResult, error) {
	...
	if err != nil {
		logx.Error("RPC调用WithdrawRecord失败，err=", err)
		return nil, err
	}
	list := records.List
	b := make([]any, len(list))
